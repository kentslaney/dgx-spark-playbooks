...
This playbook demonstrates training of [nanochat-cycled](https://github.com/kentslaney/nanochat-cycled) (nanochat with cycled RoPE positional embeddings) on DGX Station with the GB300 Ultra Superchip. You run the full pipeline on a single system: custom BPE tokenizer training, base model pretraining, supervised fine-tuning (SFT), and inference via CLI.

The project uses the PyTorch NGC container, FineWeb for pretraining data, SmolTalk + benchmark mixtures for SFT, and Weights & Biases for logging. The default configuration trains a ~1B parameter (d24) Transformer model with FP8 precision.

## What you'll accomplish

- **Environment:** Docker image with PyTorch NGC and nanochat dependencies on your DGX Station.
- **Training pipeline:** BPE tokenizer (32K vocab), base model pretraining with FP8, SFT with strided/cycled position IDs, and evaluation benchmarks.
- **Inference:** CLI to chat with the base or SFT checkpoints.
- **Monitoring:** W&B dashboards with training loss, validation BPB, and throughput metrics.
...
Vocabulary Size: 32,768 (2^15, trained BPE)
...
| Stage | Description |
|-------|-------------|
| Tokenizer | Trains BPE tokenizer (32K vocab) on ~2B characters from FineWeb |
| Base pretraining | Pretrains d24 model on FineWeb with FP8, target data:param ratio of 8 |
| SFT | Fine-tunes on SmolTalk, GSM8K, and MMLU task mixture with cycled RoPE |
| Evaluation | Generates benchmark scores (CORE, ARC, GSM8K, MMLU) across checkpoints |
...
- `setup.sh` – Clones nanochat-cycled, checks out the supported commit, copies `speedrun_station.sh`, and builds the Docker image.
- `launch.sh` – Runs the training container (full pipeline: tokenizer → pretrain → SFT → eval).
...
- [nanochat-cycled](https://github.com/kentslaney/nanochat-cycled) with cycled RoPE positional embeddings
...
Run the setup script. It clones [nanochat-cycled](https://github.com/kentslaney/nanochat-cycled), checks out the supported commit, copies the station-adapted `speedrun_station.sh`, and builds the `nanochat` Docker image (PyTorch NGC base with dependencies):
...
1. **Tokenizer training** — downloads ~2B characters from ClimbMix, trains a 32K BPE tokenizer
2. **Base model pretraining** — downloads additional ClimbMix shards, pretrains a d24 model (~1B params) with FP8
3. **SFT** — fine-tunes on task mixtures with cycled RoPE striding
4. **Evaluation** — evaluates the model on chat benchmarks (ARC, GSM8K, MMLU)
...
After training, checkpoints are saved under the `nanochat_cache/` directory. Run interactive CLI chat with the model from inside the container:
